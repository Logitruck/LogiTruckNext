...
exports.insertMessage = async data => {
  ...
  // Get the language preferences of all participants
  const participants = channel.data().participants;
  const participantLanguages = participants.map(participant => participant.language);

  // Translate the message content to each participant's language
  ...
  // Add the translated messages to the message data
...
const broadcastNotificationToAllParticipants = async (channelID, message) => {
  ...
  let content = sender.firstName

  if (downloadObject) {
    if (downloadObject?.type?.includes('video')) {
      content += ' sent a video.'
    } else if (downloadObject?.type?.includes('image')) {
      content += ' sent a photo.'
    } else if (downloadObject?.type?.includes('audio')) {
      content += ' sent an audio.'
    } else if (downloadObject?.type?.includes('file')) {
      content += ' sent a file.'
    }
  } else {
    if (isGroupChat) {
      content = `${sender.firstName}: ${message.content}`
    } else {
      content = message.content
    }
  }
