...
exports.getListWithFilters = async (
  docRef,
  collectionName,
  page,
  limit,
  ...
  try {
    const collectionUsed = page === -1
      ? `${collectionName}_live`
      : `${collectionName}_historical`;
      
    console.log(`[getListWithFilters] Fetching from ${collectionUsed}, page=${page}, limit=${limit}`);
    
    let query = docRef.collection(collectionUsed);

    filters.forEach(filter => {
      console.log(`[getListWithFilters] Applying filter:`, filter);
    ...
    // ✅ Solo aplica offset y limit si page !== -1
    if (page !== -1) {
      query = query.offset(page * limit).limit(limit);
    }

    const snapshot = await query.get();
    console.log(`[getListWithFilters] Retrieved ${snapshot.size} docs`);
    return snapshot?.docs?.map(doc => doc.data());

  } catch (error) {
    console.error(`[getListWithFilters] Error:`, error);
    throw error;
  }
};
